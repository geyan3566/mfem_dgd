...
private:
  ...
  */
  void InitMap(const Array<int> &attr, const Array<Coefficient *> &coefs);
...
public:
  ...
  */
  ...
  void UpdateCoefficient(int attr, Coefficient &coef) { pieces[attr] = &coef; }

  /// Remove a single Coefficient for a particular attribute
  void ZeroCoefficient(int attr) { pieces.erase(attr); }
...
public:
  ...
  virtual real_t EvalDelta(ElementTransformation &T,
...
private:
  ...
  */
  void InitMap(const Array<int> &attr, const Array<VectorCoefficient *> &coefs);
...
public:
  ...
  VectorFunctionCoefficient(
      int dim, std::function<void(const Vector &, real_t, Vector &)> TDF,
      Coefficient *q = nullptr)
...
private:
  ...
  */
  ...
  /** Convenience function to check for compatible array lengths,
      ...
      pairs to the internal data structure.
  */
  void InitMap(const Array<int> &attr, const Array<MatrixCoefficient *> &coefs);
...
public:
  ...
  MatrixFunctionCoefficient(
      int dim, std::function<void(const Vector &, DenseMatrix &)> F,
      Coefficient *q = nullptr)
  ...
  MatrixFunctionCoefficient(int dim,
                            ...
                            Coefficient *q = NULL)
      : MatrixCoefficient(dim, true), SymmFunction(std::move(SymmF)), Q(q),
        mat(0) {}
  ...
  /** \param dim - the size of the matrix
      ...
      \param q - optional scalar Coefficient to scale the matrix coefficient */
  MatrixFunctionCoefficient(
      int dim, std::function<void(const Vector &, real_t, DenseMatrix &)> TDF,
      Coefficient *q = nullptr)
...
public:
  ...
  /// point @a ip.
  real_t Eval(int i, int j, ElementTransformation &T,
              const IntegrationPoint &ip) {
    return Coeff[i * width + j] ? Coeff[i * width + j]->Eval(T, ip, GetTime())
                                : 0.0;
  }
...
public:
  ...
  /// Evaluate coefficient located at the i-th row of the matrix using
  /// integration point @a ip.
...
public:
  ...
  SumCoefficient(real_t A, Coefficient &B, real_t alpha_ = 1.0,
                 real_t beta_ = 1.0)
      : aConst(A), a(NULL), b(&B), alpha(alpha_), beta(beta_) {}

  /// Constructor with two coefficients.  Result is alpha_ * A + beta_ * B.
  SumCoefficient(Coefficient &A, Coefficient &B, real_t alpha_ = 1.0,
                 real_t beta_ = 1.0)
...
/// Base class for symmetric matrix coefficients that optionally depend on time
/// and space.
...
public:
  ...
  /// @deprecated Return a reference to the internal matrix used when evaluating
  /// this coefficient as a DenseMatrix.
...
private:
  ...
  std::function<void(const Vector &, real_t, DenseSymmetricMatrix &)>
      TDFunction;
...
public:
  ...
  SymmetricMatrixFunctionCoefficient(
      int dim, std::function<void(const Vector &, DenseSymmetricMatrix &)> F,
      Coefficient *q = nullptr)
...
public:
  ...
  ProductCoefficient(real_t A, Coefficient &B) : aConst(A), a(NULL), b(&B) {}
...
public:
  ...
  PowerCoefficient(Coefficient &A, real_t p_) : a(&A), p(p_) {}
...
/// Scalar coefficient defined as a cross product of two vectors in the
/// xy-plane.
...
public:
  /// Constructor with two vector coefficients.  Result is $ A_x B_y - A_y *
  /// B_x; $.
...
public:
  ...
  IdentityMatrixCoefficient(int d) : MatrixCoefficient(d, d), dim(d) {}
...
inline CoefficientStorage operator|(CoefficientStorage a,
...
protected:
  CoefficientStorage
      storage;             ///< Storage optimizations (see CoefficientStorage).
