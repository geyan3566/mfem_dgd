...
protected:
  /// FE space on which the grid function lives. Owned if #fec_owned is not
  /// NULL.
  ...
  void SumFluxAndCount(BilinearFormIntegrator &blfi, GridFunction &flux,
                       Array<int> &counts, bool wcoef, int subdomain);
...
public:
  ...
  GridFunction(const GridFunction &orig)
      : Vector(orig), fes(orig.fes), fec_owned(NULL),
  ...
  /** If the new FiniteElementCollection, @a fec_, is NULL, ownership of
     #fec_owned and #fes is taken away. */
  ...
  */
  ...
  void GetVectorValues(int i, const IntegrationRule &ir, DenseMatrix &vals,
  ...
  */
  ...
  virtual void GetVectorValue(ElementTransformation &T,
                              const IntegrationPoint &ip, Vector &val,
  ...
  void GetVectorFieldValues(int i, const IntegrationRule &ir, DenseMatrix &vals,
  ...
  void ImposeBounds(int i, const Vector &weights, const Vector &lo_,
                    const Vector &hi_);
  void ImposeBounds(int i, const Vector &weights, real_t min_ = 0.0,
...
public:
  ...
  virtual real_t
  ComputeElementGradError(int ielem, VectorCoefficient *exgrad,
  ...
  /// @a jump_scaling.
  virtual real_t
  ComputeDGFaceJumpError(Coefficient *exsol, Coefficient *ell_coeff,
                         class JumpScaling jump_scaling,
                         const IntegrationRule *irs[] = NULL) const;

  /// Returns the Face Jumps error for L2 elements, with 1/h scaling.
  MFEM_DEPRECATED
  real_t ComputeDGFaceJumpError(Coefficient *exsol, Coefficient *ell_coeff,
                                real_t Nu,
                                const IntegrationRule *irs[] = NULL) const;
  ...
  virtual real_t ComputeHDivError(VectorCoefficient *exsol, Coefficient *exdiv,
                                  const IntegrationRule *irs[] = NULL) const;
  ...
  virtual void
  ComputeElementLpErrors(const real_t p, Coefficient &exsol, Vector &error,
                         Coefficient *weight = NULL,
                         const IntegrationRule *irs[] = NULL) const;

  virtual void
  ComputeElementL1Errors(Coefficient &exsol, Vector &error,
                         const IntegrationRule *irs[] = NULL) const {
    ComputeElementLpErrors(1.0, exsol, error, NULL, irs);
  }

  virtual void
  ComputeElementL2Errors(Coefficient &exsol, Vector &error,
                         const IntegrationRule *irs[] = NULL) const {
    ComputeElementLpErrors(2.0, exsol, error, NULL, irs);
  }

  virtual void
  ComputeElementMaxErrors(Coefficient &exsol, Vector &error,
                          const IntegrationRule *irs[] = NULL) const {
  ...
  /** Compute the Lp error in each element of the mesh and store the results in
      ...
      for example an L2 GridFunction of order zero using map type VALUE. */
  virtual void
  ComputeElementLpErrors(const real_t p, VectorCoefficient &exsol,
                         Vector &error, Coefficient *weight = NULL,
                         VectorCoefficient *v_weight = NULL,
                         const IntegrationRule *irs[] = NULL) const;

  virtual void
  ComputeElementL1Errors(VectorCoefficient &exsol, Vector &error,
                         const IntegrationRule *irs[] = NULL) const {
    ComputeElementLpErrors(1.0, exsol, error, NULL, NULL, irs);
  }

  virtual void
  ComputeElementL2Errors(VectorCoefficient &exsol, Vector &error,
                         const IntegrationRule *irs[] = NULL) const {
    ComputeElementLpErrors(2.0, exsol, error, NULL, NULL, irs);
  }

  virtual void
  ComputeElementMaxErrors(VectorCoefficient &exsol, Vector &error,
                          const IntegrationRule *irs[] = NULL) const {
  ...
  virtual void Save(adios2stream &out, const std::string &variable_name,
                    const adios2stream::data_type type =
                        adios2stream::data_type::point_data) const;
#endif
...
real_t ZZErrorEstimator(BilinearFormIntegrator &blfi, GridFunction &u,
                        GridFunction &flux, Vector &error_estimates,
                        Array<int> *aniso_flags = NULL, int with_subdomains = 1,
                        bool with_coeff = false);

/// Defines the global tensor product polynomial space used by
/// NewZZErorrEstimator
/**
...
/**
 *  By default, BoundingBox(...) computes the parameters of a minimal bounding
 * box for the given @a face_patch that is aligned with the physical (i.e.
 * global) Cartesian axes. This means that the size of the bounding box will
 * depend on the orientation of the patch. It is better to construct an
 * orientation-independent box. This is implemented for 2D patches. The
 * parameters @a angle and @a midpoint encode the necessary additional geometric
 * information.
 *
...
/// A ``true'' ZZ error estimator that uses face-based patches for flux
/// reconstruction.
/**
 *  Only two-element face patches are ever used:
 *   - For conforming faces, the face patch consists of its two neighboring
 * elements.
 *   - In the non-conforming setting, only the face patches associated to
 * fine-scale element faces are used. These face patches always consist of two
 * elements delivered by mesh::GetFaceElements(Face, *Elem1, *Elem2).
 */
real_t
LSZZErrorEstimator(BilinearFormIntegrator &blfi,         // input
...
real_t ComputeElementLpDistance(real_t p, int i, GridFunction &gf1,
                                GridFunction &gf2);
...
GridFunction *Extrude1DGridFunction(Mesh *mesh, Mesh *mesh2d, GridFunction *sol,
                                    const int ny);
