...
FiniteElementSpace::FiniteElementSpace()
    : mesh(NULL), fec(NULL), vdim(0), ordering(Ordering::byNODES), ndofs(0),
      nvdofs(0), nedofs(0), nfdofs(0), nbdofs(0), bdofs(NULL), elem_dof(NULL),
      elem_fos(NULL), bdr_elem_dof(NULL), bdr_elem_fos(NULL), face_dof(NULL),
      NURBSext(NULL), own_ext(false), cP_is_set(false), Th(Operator::ANY_TYPE),
...
void FiniteElementSpace::CopyProlongationAndRestriction(
  ...
  if (fes.GetConformingProlongation() != NULL) {
    if (perm) {
      cP.reset(Mult(*perm_mat, *fes.GetConformingProlongation()));
    } else {
      cP.reset(new SparseMatrix(*fes.GetConformingProlongation()));
    }
    cP_is_set = true;
  } else if (perm != NULL) {
    cP.reset(perm_mat);
  ...
  if (fes.GetConformingRestriction() != NULL) {
    if (perm) {
      cR.reset(Mult(*fes.GetConformingRestriction(), *perm_mat_tr));
    } else {
      cR.reset(new SparseMatrix(*fes.GetConformingRestriction()));
    }
  } else if (perm != NULL) {
...
void FiniteElementSpace::DofsToVDofs(int vd, Array<int> &dofs,
                                     int ndofs_) const {
  if (vdim == 1) {
    return;
  }
...
void FiniteElementSpace::GetEssentialTrueDofs(const Array<int> &bdr_attr_is_ess,
  ...
  if (!R) {
    ...
    MFEM_VERIFY(counter == 0, "internal MFEM error: counter = "
                                  << counter << ' ' << error_msg);
#endif
  }
...
void FiniteElementSpace::AddDependencies(SparseMatrix &deps,
...
void FiniteElementSpace::AddEdgeFaceDependencies(
  ...
  for (int i = 0; i < E.Size(); i++) {
    ...
    if (mid[0] > eps && mid[0] < 1 - eps && mid[1] > eps && mid[1] < 1 - eps) {
...
int FiniteElementSpace::GetNumBorderDofs(Geometry::Type geom, int order) const {
  ...
  return Geometry::NumVerts[geom] *
         (geom == Geometry::SEGMENT ? nv : (nv + ne));
}
...
void FiniteElementSpace::BuildConformingInterpolation() const {
    ...
    for (const NCMesh::Master &master : list.masters) {
      ...
      for (int si = master.slaves_begin; si < master.slaves_end; si++) {
        ...
        if (skipfirst) {
          ...
          AddEdgeFaceDependencies(deps, master_dofs, master_fe, slave_dofs,
                                  slave.index, pm);
        }
      }
      ...
      if (IsVariableOrder()) {
        ...
        if (nvar > 1) {
          int q = GetEntityDofs(entity, master.index, highest_dofs, master_geom,
                                nvar - 1);
  ...
  MFEM_VERIFY(n_finalized == ndofs, "Error creating cP matrix: n_finalized = "
                                        << n_finalized
                                        << ", ndofs = " << ndofs);
...
const SparseMatrix *FiniteElementSpace::GetConformingRestriction() const {
  ...
  if (cR && !R_transpose) {
    R_transpose.reset(new TransposeOperator(*cR));
  }
  return cR.get();
}
...
const ElementRestrictionOperator *
FiniteElementSpace::GetElementRestriction(ElementDofOrdering e_ordering) const {
...
const FaceRestriction *
FiniteElementSpace::GetFaceRestriction(ElementDofOrdering f_ordering,
                                       FaceType type, L2FaceValues mul) const {
...
const QuadratureInterpolator *
FiniteElementSpace::GetQuadratureInterpolator(const QuadratureSpace &qs) const {
  for (int i = 0; i < E2Q_array.Size(); i++) {
    const QuadratureInterpolator *qi = E2Q_array[i];
    if (qi->qspace == &qs) {
      return qi;
...
FiniteElementSpace::GetFaceQuadratureInterpolator(const IntegrationRule &ir,
                                                  FaceType type) const {
...
void FiniteElementSpace::GetLocalRefinementMatrices(Geometry::Type geom,
...
SparseMatrix *FiniteElementSpace::RefinementMatrix(int old_ndofs,
  ...
  MFEM_VERIFY(GetNE() >= old_elem_dof->Size(), "Previous mesh is not coarser.");
...
FiniteElementSpace::RefinementOperator::RefinementOperator(
    ...
    int old_ndofs)
    : fespace(fespace), old_elem_dof(old_elem_dof), old_elem_fos(old_elem_fos) {
...
FiniteElementSpace::RefinementOperator::RefinementOperator(
    ...
    : Operator(fespace->GetVSize(), coarse_fes->GetVSize()), fespace(fespace),
      old_elem_dof(NULL), old_elem_fos(NULL) {
  Mesh::GeometryList elem_geoms(*fespace->GetMesh());
...
void FiniteElementSpace::RefinementOperator::Mult(const Vector &x,
  ...
  for (int k = 0; k < mesh_ref->GetNE(); k++) {
    ...
    const DenseMatrix &lP =
        (fespace->IsVariableOrder()) ? eP : localP[geom](emb.matrix);
...
struct RefType {
  ...
  bool operator<(const RefType &other) const {
    ...
    for (int i = 0; i < num_children; i++) {
      if (children[i].one < other.children[i].one) {
        return true;
      }
      if (children[i].one > other.children[i].one) {
        return false;
...
void GetCoarseToFineMap(const CoarseFineTransformations &cft,
  ...
  for (int i = 0; i < coarse_ne; i++) {
    ...
    pair<map<RefType, int>::iterator, bool> res = ref_type_map.insert(
...
FiniteElementSpace::DerefinementOperator::DerefinementOperator(
    ...
    : Operator(c_fes->GetVSize(), f_fes->GetVSize()), fine_fes(f_fes) {
  ...
  for (int gi = 0; gi < elem_geoms.Size(); gi++) {
    ...
    const FiniteElement *fine_fe = f_fes->fec->FiniteElementForGeometry(geom);
    const FiniteElement *coarse_fe = c_fes->fec->FiniteElementForGeometry(geom);
    const DenseTensor &pmats = rtrans.point_matrices[geom];
...
void FiniteElementSpace::GetLocalDerefinementMatrices(
    Geometry::Type geom, DenseTensor &localR) const {
  const FiniteElement *fe = fec->FiniteElementForGeometry(geom);
...
SparseMatrix *
FiniteElementSpace::DerefinementMatrix(int old_ndofs, const Table *old_elem_dof,
                                       const Table *old_elem_fos) {
  /// TODO: Implement DofTransformation support
  ...
  bool is_dg =
      FEColl()->GetContType() == FiniteElementCollection::DISCONTINUOUS;
  int num_marked = 0;
  const FiniteElement *fe = nullptr;
...
int FiniteElementSpace::FindDofs(const Table &var_dof_table, int row,
                                 int ndof) const {
...
void FiniteElementSpace::GetElementDofs(int elem, Array<int> &dofs,
  ...
  if (nfd) // face DOFs
  {
    for (int i = 0; i < F.Size(); i++) {
      ...
      int fbase =
          (var_face_dofs.Size() > 0) ? FindFaceDof(F[i], nf) : F[i] * nf;
...
const FiniteElement *
FiniteElementSpace::GetTraceElement(int i, Geometry::Type geom_type) const {
  return fec->TraceFiniteElementForGeometry(geom_type);
}

FiniteElementSpace::~FiniteElementSpace() { Destroy(); }
...
void FiniteElementSpace::GetTransferOperator(
  ...
  if (T.Type() == Operator::MFEM_SPARSEMAT) {
    if (!IsVariableOrder()) {
      ...
      T.Reset(RefinementMatrix_main(
          coarse_fes.GetNDofs(), coarse_fes.GetElementToDofTable(),
          coarse_fes.GetElementToFaceOrientationTable(), localP));
    } else {
...
void FiniteElementSpace::GetTrueTransferOperator(
  ...
  if (req_type == Operator::MFEM_SPARSEMAT) {
    ...
    case 2:
      ...
      break;
    case 3:
      T.Reset(new TripleProductOperator(cR.get(), T.Ptr(), coarse_P, false,
                                        owner, false));
      break;
    }
  }
  ...
  if (want_transform) {
    ...
    case Mesh::REFINE: {
      if (Th.Type() != Operator::MFEM_SPARSEMAT) {
        Th.Reset(new RefinementOperator(this, old_elem_dof, old_elem_fos,
                                        old_ndofs));
...
void FiniteElementSpace::UpdateMeshPointer(Mesh *new_mesh) { mesh = new_mesh; }
...
void FiniteElementSpace::Save(std::ostream &os) const {
  ...
  os << (fes_format == 90 ? "FiniteElementSpace\n"
...
FiniteElementCollection *FiniteElementSpace::Load(Mesh *m,
                                                  std::istream &input) {
  string buff;
  ...
  if (buff == "FiniteElementSpace") {
    fes_format = 90; /* v0.9 */
  } else if (buff == "MFEM FiniteElementSpace v1.0") {
    fes_format = 100;
  } else {
    MFEM_ABORT("input stream is not a FiniteElementSpace!");
  }
  ...
  } else if (fes_format == 100) // v1.0
  {
    while (1) {
      ...
      if (buff == "NURBS_order" || buff == "NURBS_orders") {
        MFEM_VERIFY(nurbs_fec, buff << ": NURBS FE collection is required!");
...
ElementDofOrdering GetEVectorOrdering(const FiniteElementSpace &fes) {
  return UsesTensorBasis(fes) ? ElementDofOrdering::LEXICOGRAPHIC
