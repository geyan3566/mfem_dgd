...
template <class T>
adios2::Variable<T>
SafeDefineVariable(adios2::IO io, const std::string &variable_name,
...
template <class T>
adios2::Attribute<T>
SafeDefineAttribute(adios2::IO io, const std::string &attribute_name,
                    const T &value, const std::string &variable_name = "",
...
adios2stream::adios2stream(const std::string &name, const openmode mode,
    ...
    : name(name), adios2_openmode(mode), adios(new adios2::ADIOS(comm)),
...
adios2stream::adios2stream(const std::string &name, const openmode mode,
    ...
    : name(name), adios2_openmode(mode), adios(new adios2::ADIOS()),
      io(adios->DeclareIO(name)) {
  io.SetEngine(engineType);
}
#endif
...
size_t adios2stream::CurrentStep() const { return engine.CurrentStep(); }
...
void adios2stream::Print(const Mesh &mesh, const mode print_mode) {
  auto lf_DefineMeshMetadata = [this](Mesh &mesh) {
    ...
    if (refine) {
      for (int i = 0; i < mesh.GetNE(); ++i) {
        ...
        RefinedGeometry *refined_geometry =
            GlobGeometryRefiner.Refine(type, refinement_level, 1);
        if (refined_geometry == nullptr) {
          mfem_error("ERROR: could not refine geometry in call to Save with "
                     "adios2stream \n");
        }

        element_nvertices =
            static_cast<size_t>(Geometries.GetVertices(type)->GetNPoints());
    ...
    SafeDefineVariable<uint64_t>(io, "connectivity", {}, {},
                                 {nelements, element_nvertices + 1});
    SafeDefineVariable<int32_t>(io, "attribute", {}, {}, {nelements});

    // vertices
    SafeDefineVariable<uint32_t>(io, "NumOfVertices", {adios2::LocalValueDim});
    if (refine) {
      SafeDefineVariable<double>(io, "vertices", {}, {},
                                 {nvertices, static_cast<size_t>(dimension)});
    } else {
  ...
  auto lf_PrintRefinedMeshData = [this](Mesh &mesh) {
    ...
    adios2::Variable<double> var_vertices =
        io.InquireVariable<double>("vertices");
    adios2::Variable<double>::Span span_vertices =
        engine.Put<double>(var_vertices);
    ...
    adios2::Variable<uint64_t>::Span span_connectivity =
        engine.Put<uint64_t>(var_connectivity);
    ...
    for (int e = 0; e < mesh.GetNE(); ++e) {
      ...
      RefinedGeometry *refined_geometry =
          GlobGeometryRefiner.Refine(type, refinement_level, 1);
      // vertices
      mesh.GetElementTransformation(e)->Transform(refined_geometry->RefPts,
                                                  pmatrix);
      for (int i = 0; i < pmatrix.Width(); ++i) {
        for (int j = 0; j < pmatrix.Height(); ++j) {
          span_vertices[span_vertices_offset + i * pmatrix.Height() + j] =
              pmatrix(j, i);
        }
      }
      span_vertices_offset +=
          static_cast<size_t>(pmatrix.Width() * pmatrix.Height());
      ...
      for (int v = 0; v < element_vertices.Size();) {
        ...
        span_element_attribute[span_element_attribute_offset] =
            static_cast<int32_t>(element_attribute);
        ++span_element_attribute_offset;

        for (int k = 0; k < nv; k++, v++) {
          span_connectivity[span_connectivity_offset] =
              static_cast<uint64_t>(point_id + element_vertices[v]);
          ++span_connectivity_offset;
        }
      }
  ...
  auto lf_PrintMeshData = [&](Mesh &mesh) {
    ...
    adios2::Variable<uint64_t> varConnectivity =
        io.InquireVariable<uint64_t>("connectivity");
    // zero-copy access to adios2 buffer to put non-contiguous to contiguous
    // memory
...
void adios2stream::Save(const GridFunction &grid_function,
  ...
  auto lf_SafeDefine = [&](const std::string &variable_name,
    ...
    if (!var) {
      ...
      SafeDefineAttribute<std::string>(io, "FiniteElementSpace", fespace_name,
                                       variable_name);
    }
  };
  ...
  if (refine) {
    ...
    lf_SafeDefine(variable_name, tuples, components, Ordering::byVDIM,
    ...
    adios2::Variable<double> variable =
        io.InquireVariable<double>(variable_name);
    ...
    if (components == 1) {
      ...
      for (int e = 0; e < mesh->GetNE(); ++e) {
        refined_geometry = GlobGeometryRefiner.Refine(
            mesh->GetElementBaseGeometry(e), refinement_level, 1);
        grid_function.GetVectorValues(e, refined_geometry->RefPts, vector,
                                      transform);
  ...
  if (full_data) {
    ...
    lf_SafeDefine(variable_name + "/full", tuples, components,
                  fes->GetOrdering(), std::string(fes->FEColl()->Name()));
    // calls Vector::Print
...
// PRIVATE
int32_t adios2stream::GLVISToVTKType(const int glvisType) const noexcept {
  uint32_t vtkType = 0;
  switch (glvisType) {
...
bool adios2stream::IsConstantElementType(
    const Array<Element *> &elements) const noexcept {
...
adios2::Mode
adios2stream::ToADIOS2Mode(const adios2stream::openmode mode) const noexcept {
